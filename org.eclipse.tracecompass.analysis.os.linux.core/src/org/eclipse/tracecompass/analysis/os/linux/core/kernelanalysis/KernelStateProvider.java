...
public class KernelStateProvider extends AbstractTmfStateProvider {
    ...
    protected void eventHandle(@Nullable ITmfEvent event) {
        ...
        try {
             ...
             * transition.
             */
            Integer idx = fEventNames.get(getEventName(event));
            int intval = (idx == null ? -1 : idx.intValue());
            switch (intval) {

            case IRQ_HANDLER_ENTRY_INDEX:
                handleIrqHandlerEntry(ss, event);
                break;

            case IRQ_HANDLER_EXIT_INDEX:
                handleIrqHandlerExit(ss, event);
                break;

            case SOFT_IRQ_ENTRY_INDEX:
                handleSoftIrqHandlerEntry(ss, event);
                break;

            case SOFT_IRQ_EXIT_INDEX:
                handleSoftIrqExit(ss, event);
                break;

            case SOFT_IRQ_RAISE_INDEX:
                handleSoftIrqRaise(ss, event);
                break;

            case SCHED_SWITCH_INDEX:
                handleSchedSwitch(ss, event);
                break;

            case SCHED_PROCESS_FORK_INDEX:
                handleSchedProcessFork(ss, event);
                break;
    ...
    private static @Nullable Integer getCpu(ITmfEvent event) {
        return (Integer) TmfTraceUtils.resolveEventAspectOfClassForEvent(event.getTrace(), TmfCpuAspect.class, event);
    }

    private static int getCurrentThreadNode(final ITmfStateSystemBuilder ss, final int currentCPUNode) throws AttributeNotFoundException {
        /*
    ...
    private static int getCpuNode(Integer cpu, final ITmfStateSystemBuilder ss) {
        return ss.getQuarkRelativeAndAdd(getNodeCPUs(ss), cpu.toString());
    }

    private static String getEventName(ITmfEvent event) {
        return NonNullUtils.checkNotNull(event.getType().getName());
    }

    private static long getTime(ITmfEvent event) {
     ...
     */
    private static void handleSyscallEntry(final ITmfStateSystemBuilder ss, final ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        String eventName = getEventName(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
        /* Assign the new system call to the process */
        int quark = ss.getQuarkRelativeAndAdd(currentThreadNode, Attributes.SYSTEM_CALL);
        ITmfStateValue value = TmfStateValue.newValueString(eventName);
     ...
     *             an exception if the attribute it not found
     */
    private static void handleSyscallExit(final ITmfStateSystemBuilder ss, final ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
     ...
     *             an exception if the attribute it not found
     */
    private void handleSchedWakeup(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        ...
        if (status != StateValues.PROCESS_STATUS_RUN_SYSCALL &&
            ...
            ss.modifyAttribute(ts, StateValues.PROCESS_STATUS_WAIT_FOR_CPU_VALUE, quark);
        }
    }
     ...
     *             an exception if the attribute it not found
     */
    private void handleSchedProcessFork(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
     ...
     *             an exception if the attribute it not found
     */
    private void handleSchedProcessFree(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        ...
        ss.removeAttribute(getTime(event), quark);
    }
     ...
     *             an exception if the attribute it not found
     */
    private void handleSchedSwitch(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        ITmfEventField content = event.getContent();
     ...
     *             an exception if the attribute it not found
     */
    private void handleSoftIrqRaise(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        /* Fields: int32 vec */
        ...
        ss.modifyAttribute(ts, StateValues.SOFT_IRQ_RAISED_VALUE, quark);
    }
     ...
     *             an exception if the attribute it not found
     */
    private void handleSoftIrqHandlerEntry(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
     ...
     *             an exception if the attribute it not found
     */
    private void handleSoftIrqExit(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
     ...
     *             an exception if the attribute it not found
     */
    private void handleIrqHandlerEntry(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
     ...
     *             an exception if the attribute it not found
     */
    private void handleIrqHandlerExit(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        Integer cpu = checkNotNull(getCpu(event));
        int currentCPUNode = getCpuNode(cpu, ss);
        int currentThreadNode = getCurrentThreadNode(ss, currentCPUNode);
     ...
     *             an exception if the attribute it not found
     */
    private static void handleLttngStatedumpProcessState(final ITmfStateSystemBuilder ss, ITmfEvent event) throws AttributeNotFoundException {
        long ts = getTime(event);
        /* LTTng-specific */
        ITmfEventField content = event.getContent();
