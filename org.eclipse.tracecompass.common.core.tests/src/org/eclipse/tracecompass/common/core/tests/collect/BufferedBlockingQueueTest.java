...
public class BufferedBlockingQueueTest {
    ...
    private static final String testString = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz";
    ...
    public void testLargeInsertion() {
        String string = testString.substring(0, 100);
        for (char elem : string.toCharArray()) {
            charQueue.put(elem);
        }
        charQueue.flushInputBuffer();
     ...
     */
    private static <T> boolean containsSameElements(Collection<T> c1, Collection<T> c2) {
        return HashMultiset.create(c1).equals(HashMultiset.create(c2));
    }
