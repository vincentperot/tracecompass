...
public class BufferedBlockingQueueTest {
    ...
    private static List<String> generateTestVector(int size, int wordSize) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < wordSize; j++) {
                element.append(values.get(letterGenerator.nextInt(values.size())));
            }
    ...
    public void testSimpleInsertion() {
        Iterable<String> string = generateTestVector(10, 5);
    ...
    public void testLargeInsertion() {
        Iterable<String> string = generateTestVector(100, 4);
        StringBuilder expected = new StringBuilder();
        for (String elem : string) {
            fStringQueue.put(NonNullUtils.checkNotNull(elem));
    ...
    public void testMultiThread2Producers() throws InterruptedException {
        /* A character not found in the test string */
        final List<String> testString = generateTestVector(1000, 10);
        final String lastElement = "%";
        ...
        assertEquals(testString.size() * 2, actual.size());
        Collection<String> doubleList = new ArrayList<>();
        doubleList.addAll(testString);
        doubleList.addAll(testString);
        assertEquals(HashMultiset.create(doubleList), HashMultiset.create(actual));
    }
    ...
    public void testMultiThreadManyProducers() throws InterruptedException {
        ...
        /* A character not found in the test string */
        final List<String> testString = generateTestVector(150, 9);
        final String lastElement = "!";
        Thread.currentThread().setName("Unit test");
    ...
    public void testIteratorContents() {
        ...
        containsSameElements(expected, actual);
    ...
    /**
     * Utility method to test that two collection contain the exact same
     ...
     */
    private static <T> void containsSameElements(Collection<T> c1, Collection<T> c2) {
