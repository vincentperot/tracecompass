...
import org.eclipse.tracecompass.analysis.graph.core.base.TmfGraphVisitor;
import org.eclipse.tracecompass.analysis.graph.core.base.TmfVertex;
import org.eclipse.tracecompass.analysis.graph.core.criticalpath.AlgorithmManager;
import org.eclipse.tracecompass.analysis.graph.core.criticalpath.CriticalPathModule;
import org.eclipse.tracecompass.analysis.graph.core.criticalpath.ICriticalPathAlgorithm;
import org.eclipse.tracecompass.analysis.graph.ui.Activator;
...
public class CriticalPathView extends AbstractTimeGraphView {
    ...
    private class CriticalPathContentProvider implements ITimeGraphContentProvider, ITreeContentProvider {
        ...
        public @Nullable ITimeGraphEntry[] getElements(@Nullable Object inputElement) {
            if (inputElement != null) {
                try {
                    ...
                    Object worker = ((CriticalPathBaseEntry) obj).getWorker();
                    fCurrentObject = worker;
                    Map<Object, CriticalPathEntry> entries = workerMaps.get(worker);
                    if (entries != null) {
                        return entries.values().toArray(new ITimeGraphEntry[entries.size()]);
                    }
                    entries = buildEntryList(worker);
                    if (entries == null) {
                        return new ITimeGraphEntry[0];
                    }
                    workerMaps.put(worker, entries);
                    return entries.values().toArray(new ITimeGraphEntry[entries.size()]);
        ...
        private @Nullable Map<Object, CriticalPathEntry> buildEntryList(Object worker) {
    ...
    public void analysisStarted(TmfStartAnalysisSignal signal) {
        ...
        TmfGraphStatistics stats = fObjectStatistics.get(getTrace(), obj);
        if (stats == null) {
            stats = new TmfGraphStatistics();
            fObjectStatistics.put(getTrace(), obj, stats);
        }
        fStats = stats;

        TimeGraphEntry tge = new CriticalPathBaseEntry(obj);
