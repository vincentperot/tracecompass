...
public class HistoryTreeBackendTest {
    ...
    public void before() throws IOException, AttributeNotFoundException {
        IStateHistoryBackend backend = StateHistoryBackendFactory.createHistoryTreeBackendExistingFile("test", checkNotNull(testHtFile), 0);
     ...
     * <p>
     * Tests that getFullAttributePath(getQuarkAbsolute(array)))).equals(array).
     * <p>
     * Tests that getQuarkAbsolute(getFullAttributePath(quark)).equals(quark).
     */
    @Test
    public void testPathInverseComposition() {
        for (String name : NAMES) {
            try {
                ...
                assertArrayEquals(new String[] { THREADS, name, STATUS }, path);
                try {
                    assertEquals(quark, ss.getQuarkAbsolute(path));
                } catch (AttributeNotFoundException e) {
                    fail("AttributeNotFoundException path=" + Arrays.toString(path));
                }
            } catch (AttributeNotFoundException e) {
                fail("AttributeNotFoundException name=" + name);
            }
