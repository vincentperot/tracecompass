...
public class TmfGraph {
    ...
    public TmfVertex getHead(TmfVertex vertex) {
        ...
        TmfVertex node = headNode.incomingHorizontalEdge();
        while (node != null) {
            headNode = node;
            node = headNode.incomingHorizontalEdge();
        }
        return headNode;
    }
    ...
    public void scanLineTraverse(final @Nullable TmfVertex start, final ITmfGraphVisitor visitor) {
        ...
        while (!stack.isEmpty()) {
            ...
            while (n != null) {
                ...
                if (n.outgoingVerticalEdge() != null) {
                    stack.push(n.outgoingVerticalEdge());
                    TmfEdge edge = n.getEdges().get(TmfVertex.OUTGOING_VERTICAL_EDGE);
                    if (edge != null) {
                        visitor.visit(edge, false);
                    }
                }
                if (n.incomingVerticalEdge() != null) {
                    stack.push(n.incomingVerticalEdge());
                }
                if (n.outgoingHorizontalEdge() != null) {
                    TmfEdge edge = n.getEdges().get(TmfVertex.OUTGOING_HORIZONTAL_EDGE);
                    if (edge != null) {
                        visitor.visit(edge, true);
                    }
                }
                visited.add(n);
                n = n.outgoingHorizontalEdge();
            }
        }
    }
