...
public class BufferedBlockingQueue<T> implements Iterable<T> {
    ...
    public boolean isEmpty() {
        // we hope this short-circuits in the first two queues, which
        // should happen
        if( fInputBuffer.isEmpty() && fOutputBuffer.isEmpty() ){
            if( fInnerQueue.isEmpty()) {
                return true;
            }
            /*
             * Should not happen often, this just tests a corner case
             */
            for(Deque<T> chunk : fInnerQueue ){
                if( !chunk.isEmpty()) {
                    return false;
                }
            }
