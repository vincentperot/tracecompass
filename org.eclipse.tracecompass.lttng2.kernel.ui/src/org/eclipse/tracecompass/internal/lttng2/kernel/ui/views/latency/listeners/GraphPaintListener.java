...
import org.eclipse.tracecompass.tmf.ui.views.distribution.model.BaseDistributionData;
...
public class GraphPaintListener extends AbstractPaintListener {
    ...
    public String formatStringForHorizontalAxis(long value) {
        ...
        return ts.toString(TmfTimestampFormat.getDefaulIntervalFormat());
    }
    ...
    public String formatToolTipLabel(int x, int y) {
        ...
        if (index != BaseDistributionData.OUT_OF_RANGE_BUCKET && yIndex != BaseDistributionData.OUT_OF_RANGE_BUCKET) {
            if (fScaledData.getEventCount(index, yIndex) > 0) {
                ...
                ts = new TmfTimestamp(fScaledData.getHorBucketEndTime(index), ITmfTimestamp.NANOSECOND_SCALE);
                buffer.append(ts.toString(TmfTimestampFormat.getDefaulIntervalFormat()));
                buffer.append("]\n"); //$NON-NLS-1$
                ...
                ts = new TmfTimestamp(yStartTime, ITmfTimestamp.NANOSECOND_SCALE);
                buffer.append(ts.toString(TmfTimestampFormat.getDefaulIntervalFormat()));
                buffer.append(","); //$NON-NLS-1$
                ts = new TmfTimestamp(fScaledData.getVerBucketEndTime(yIndex), ITmfTimestamp.NANOSECOND_SCALE);
                buffer.append(ts.toString(TmfTimestampFormat.getDefaulIntervalFormat()));
                buffer.append("]\n"); //$NON-NLS-1$
     ...
     * @return index from horizontal (x) position
     */
     ...
     * @return index from vertical (y) position
     */
    ...
    /**
     * @param x The horizontal position
     * @return current time from horizontal position
     */
