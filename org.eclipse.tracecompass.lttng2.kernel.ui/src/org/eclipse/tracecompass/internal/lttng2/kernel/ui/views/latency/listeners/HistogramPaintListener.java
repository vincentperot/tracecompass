 ...
 *******************************************************************************/
package org.eclipse.tracecompass.internal.lttng2.kernel.ui.views.latency.listeners;
...
import org.eclipse.tracecompass.internal.lttng2.kernel.ui.views.latency.HistogramViewer;
import org.eclipse.tracecompass.internal.lttng2.kernel.ui.views.latency.Messages;
...
public class HistogramPaintListener extends AbstractPaintListener {
        ...
        if (fYMin >= 0L && fYMax != 0L) {
            ...
            while (multipleSameValues) {
                ...
                for (int i = 0; i < nbTicks; i++) {
                    double currentValue = (fYMin + i * valueStep) / (Math.pow(10, fDelta));
    ...
    public void paintContent() {
        ...
        for (int i = 0; i < fScaledData.fData.length; i++) {
            double pointY = fScaledData.fData[i].getNbEvents(); // TODO draw per trace
            ...
            double barHeight = zoomFactor * ((pointY - fYMin) / (fYMax - fYMin)) * height;
            ...
            if (barHeight > 0) {
                ...
                if (barHeight > height - 1) {
                    fAxisImage.fillRectangle((int) x, (int) y, barWidth, (int) (barHeight + 1));
                } else {
                    fAxisImage.fillRectangle((int) x, (int) y, barWidth, (int) (barHeight + 2));
                }
            }
        }
    ...
    public String formatToolTipLabel(int x, int y) {
        if (fScaledData != null) {
            ...
            if (index >= 0 && index <= fScaledData.fLastBucket) {
                barHeight = (zoomFactor * height * (fScaledData.fData[index].getNbEvents() - fYMin) / (fYMax - fYMin)); // TODO draw per trace
            }
...
//                fScaledData.fCurrentBucket = index;
                ...
                int nbEvents = fScaledData.fData[index].getNbEvents(); // TODO draw per trace

                StringBuffer buffer = new StringBuffer();
                buffer.append("Latency Range in s = ["); //$NON-NLS-1$
                ITmfTimestamp ts = new TmfTimestamp(startTime, ITmfTimestamp.NANOSECOND_SCALE);
                buffer.append(ts.toString(TmfTimestampFormat.getDefaulIntervalFormat()));
                buffer.append(","); //$NON-NLS-1$
                ts = new TmfTimestamp(endTime, ITmfTimestamp.NANOSECOND_SCALE);
                buffer.append(ts.toString(TmfTimestampFormat.getDefaulIntervalFormat()));
                buffer.append("]\n"); //$NON-NLS-1$
