...
public class TmfPieChartViewer extends Composite {
    ...
    private void initContent() {
        ...
        setCurrentState(new PieChartViewerStateNoContentSelected());
    ...
    public void updateGlobalPieChart() {
        if (getGlobalPC() == null) {
            ...
            getGlobalPC().getAxisSet().getXAxis(0).getTitle().setText(""); //Hide the title over the legend //$NON-NLS-1$
            getGlobalPC().getLegend().setVisible(true);
            getGlobalPC().addListener(SWT.MouseMove, mouseListenerSelection);
        }
    ...
    /**
     * Nested class used to handle and sort more easily the pair (Name, Number of
     * occurences)
     *
