...
public class TmfStateSystemViewer extends AbstractTmfTreeViewer {
    ...
    private class StateEntry extends TmfTreeViewerEntry {
        ...
        public StateEntry(String name, int quark, String[] path, ITmfStateValue value, @NonNull TmfTimestamp start, @NonNull TmfTimestamp end, boolean modified) {
            super(name);
            fQuark = quark;
            fFullPath = (path != null) ? StateSystemUtils.pathToString(path) : null;
            fStart = start;
            fEnd = end;
            fValue = value;
            fModified = modified;
