...
public class TimeEvent implements ITimeEvent2 {
    ...
    @Override
    public ITimeEvent splitBefore(long time) {
        return (time > fTime ?
                new TimeEvent(fEntry, fTime, Math.min(fDuration, time - fTime), fValue) :
                null);
    }

    @Override
    public ITimeEvent splitAfter(long time) {
        return (time < fTime + fDuration ?
                new TimeEvent(fEntry, Math.max(fTime, time), fDuration - Math.max(0, time - fTime), fValue) :
                null);
    }
