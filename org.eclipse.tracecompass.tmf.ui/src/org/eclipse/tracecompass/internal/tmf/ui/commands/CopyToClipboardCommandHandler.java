...
public class CopyToClipboardCommandHandler extends AbstractHandler {
    ...
    public static final String COMMAND_ID = "org.eclipse.tracecompass.tmf.ui.copy_to_clipboard"; //$NON-NLS-1$
    /** Id of the columns variable. List of event table columns. */
    public static final String COLUMNS_VAR = "org.eclipse.tracecompass.tmf.ui.copy_to_clipboard.columns"; //$NON-NLS-1$
    /** Id of the start variable. Event rank of the selection start. */
    public static final String START_VAR = "org.eclipse.tracecompass.tmf.ui.copy_to_clipboard.start"; //$NON-NLS-1$
    /** Id of the end variable. Event rank of the selection end. */
    public static final String END_VAR = "org.eclipse.tracecompass.tmf.ui.copy_to_clipboard.end"; //$NON-NLS-1$
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        List<TmfEventTableColumn> columns = getColumns(applicationContext);
        Long start = getStart(applicationContext);
        Long end = getEnd(applicationContext);
        ITmfTrace trace = TmfTraceManager.getInstance().getActiveTrace();
        ITmfFilter filter = TmfTraceManager.getInstance().getCurrentTraceContext().getFilter();
        if (trace != null && columns != null && start != null && end != null) {
