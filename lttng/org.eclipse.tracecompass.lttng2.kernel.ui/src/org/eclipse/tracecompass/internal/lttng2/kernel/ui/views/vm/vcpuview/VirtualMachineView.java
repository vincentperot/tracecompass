...
public class VirtualMachineView extends AbstractTimeGraphView {
    ...
    protected void buildEventList(ITmfTrace trace, ITmfTrace parentTrace, IProgressMonitor monitor) {
        ...
        while (!complete) {
            ...
            if (groupEntry == null) {
                groupEntry = new VirtualMachineViewEntry.VmEntryBuilder(vmExperiment.getName(), startTime, endTime, vmExperiment)
                        .build();
                /*
            ...
            try {
                ...
                for (Integer vmQuark : vmQuarks) {
                    ...
                    if (vmEntry == null) {
                        vmEntry = new VirtualMachineViewEntry.VmEntryBuilder(vmName, startTime, endTime, vmExperiment)
                                .setId(vmHostId)
                                .setVmName(vmName)
                    ...
                    for (Integer vCpuQuark : ssq.getSubAttributes(vmQuark, false)) {
                        ...
                        if (vcpuEntry == null) {
                            vcpuEntry = new VirtualMachineViewEntry.VmEntryBuilder(vcpuId, startTime, endTime, vmExperiment)
                                    .setId(vcpuId)
                                    .setVmName(vmName)
                    ...
                    for (Integer threadId : threadIds) {
                        ...
                        oneThreadEntry = new VirtualMachineViewEntry.VmEntryBuilder(threadName, startTime, endTime, vmExperiment)
                                .setId(threadName)
                                .setVmName(vmName)
            ...
            case THREAD: {
                ...
                if (threadIntervals != null) {
                    ...
                    for (ITmfStateInterval interval : threadIntervals) {
                         ...
                         *
                         * Hint by Patrick: "The problem is that the thread
                         * intervals are sorted by start time, and drawn in that
                         * order.
                         *
                         * Given the intervals: Blue [0,10] Alpha [5,15] Red
                         * [10,20]
                         *
                         * Blue is drawn, then Alpha makes DarkBlue from [5,10]
                         * and DarkBackground from [10,15], then Red is drawn
                         * over [10,20], overwriting the DarkBackground. There
                         * is no DarkRed.
                         *
                         * For this to work you would have to draw all real
                         * states first, then all alpha states second.
                         *
                         * I think this would also have the side-effect that the
                         * find item used for tool tips would always find the
                         * real event and never the alpha event. This might be
                         * what we want. Right now the tool tip has State:
                         * (multiple).
                         *
                         * But using the Next Event button, we would skip to the
                         * next real event and not at the preemption event.
                         * Maybe not what we want.
                         *
                         * Maybe what we need is separate thread interval
                         * events:
                         *
                         * Blue [0,5] Preempted Blue [5,10] Preempted Red
                         * [10,15] Red [15,20]...
                         *
                         * The preempted events would have the real state value,
                         * but with a flag for alpha to be used in the
                         * postDrawEvent."
                         */
