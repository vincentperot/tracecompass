...
public class UstMemoryStateProvider extends AbstractTmfStateProvider {
    ...
    protected void eventHandle(ITmfEvent event) {
        ...
        switch (name) {
        case LttngUstStrings.MALLOC: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_PTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            Long size = (Long) event.getContent().getField(LttngUstStrings.FIELD_SIZE).getValue();
            setMem(event, ptr, size);
        }
            break;
        case LttngUstStrings.FREE: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_PTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            ...
            break;
        case LttngUstStrings.CALLOC: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_PTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            Long nmemb = (Long) event.getContent().getField(LttngUstStrings.FIELD_NMEMB).getValue();
            Long size = (Long) event.getContent().getField(LttngUstStrings.FIELD_SIZE).getValue();
            setMem(event, ptr, size * nmemb);
        }
            break;
        case LttngUstStrings.REALLOC: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_PTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            Long newPtr = (Long) event.getContent().getField(LttngUstStrings.FIELD_INPTR).getValue();
            Long size = (Long) event.getContent().getField(LttngUstStrings.FIELD_SIZE).getValue();
            setMem(event, ptr, ZERO);
            setMem(event, newPtr, size);
        }
            break;
        case LttngUstStrings.MEMALIGN: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_PTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            ...
            break;
        case LttngUstStrings.POSIX_MEMALIGN: {
            Long ptr = (Long) event.getContent().getField(LttngUstStrings.FIELD_OUTPTR).getValue();
            if (ZERO.equals(ptr)) {
                return;
            }
            Long size = (Long) event.getContent().getField(LttngUstStrings.FIELD_SIZE).getValue();
    ...
    private static Long getVtid(ITmfEvent event) {
        ITmfEventField field = event.getContent().getField(LttngUstStrings.CONTEXT_VTID);
        if (field == null) {
            return MINUS_ONE;
        }
    ...
    private static String getProcname(ITmfEvent event) {
        ITmfEventField field = event.getContent().getField(LttngUstStrings.CONTEXT_PROCNAME);
        if (field == null) {
            return EMPTY_STRING;
        }
    ...
    private void setMem(ITmfEvent event, Long ptr, Long size) {
        ...
        try {
            ...
            int tidMemQuark = ss.getQuarkRelativeAndAdd(tidQuark, LttngUstStrings.UST_MEMORY_MEMORY_ATTRIBUTE);

            ITmfStateValue prevMem = ss.queryOngoingState(tidMemQuark);
            /* First time we set this value */
            if (prevMem.isNull()) {
                int procNameQuark = ss.getQuarkRelativeAndAdd(tidQuark, LttngUstStrings.UST_MEMORY_PROCNAME_ATTRIBUTE);
                ...
                if (tid.equals(MINUS_ONE)) {
                    procName = LttngUstStrings.OTHERS;
                }
