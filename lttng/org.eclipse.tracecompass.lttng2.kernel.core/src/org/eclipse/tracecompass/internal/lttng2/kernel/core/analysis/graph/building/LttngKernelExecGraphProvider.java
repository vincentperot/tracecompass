    ...
    public void done() {
        ...
        for (LttngWorker k : kernelWorker) {
            ...
            for (TmfVertex node : nodes) {
                /* send -> recv */
                TmfVertex next = node.neighbor(TmfVertex.OUTGOING_HORIZONTAL_EDGE);
                if (node.hasNeighbor(TmfVertex.INCOMING_VERTICAL_EDGE) &&
                        next != null) {
                    ...
                    TmfVertex dst = next.outgoingVerticalEdge();
                    if (dst != null) {
                        TmfVertex src = NonNullUtils.checkNotNull(node.neighbor(TmfVertex.INCOMING_VERTICAL_EDGE));
                        /* unlink */
                        node.getEdges().set(TmfVertex.INCOMING_VERTICAL_EDGE, null);
                        next.getEdges().set(TmfVertex.OUTGOING_VERTICAL_EDGE, null);
