...
public final class ConditionHelpers {
    ...
    /**
     * Condition to check whether a time graph has arrows.
     *
     * @param timeGraphControl
     *            the time graph control
     * @return ICondition for verification
     */
    public static ICondition hasArrows(final TimeGraphControl timeGraphControl) {
        return new SWTBotTestCondition() {
            @Override
            public boolean test() throws Exception {
                return timeGraphControl.hasArrows();
            }
        };
    }
}
