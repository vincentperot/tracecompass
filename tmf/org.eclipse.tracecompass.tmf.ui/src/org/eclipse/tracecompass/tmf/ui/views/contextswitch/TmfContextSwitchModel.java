...
package org.eclipse.tracecompass.tmf.ui.views.contextswitch;
...
public class TmfContextSwitchModel {
    ...
    public void queryAllKernelContextSwitches() {
        for (Entry<ITmfTrace, KernelContextSwitchContainer> entry : fDataHolder.entrySet()) {
            queryTraceContextSwitches(entry.getKey());
        }
    }
    ...
    private class ModelUpdateJob extends Job {
        ...
        protected IStatus run(IProgressMonitor monitor) {
            ...
            long timeRangeOffset = Math.round(endTime - startTime / fResolution.doubleValue());

            /* Store querying intervals */
            List<TmfTimeRange> queryingIntervals = new LinkedList<>();
            for (int j = 0; j < fResolution; j++) {
                TmfTimestamp intervalBegin = new TmfTimestamp(startTime + j * timeRangeOffset);
                TmfTimestamp intervalEnd = new TmfTimestamp(startTime + (j + 1) * timeRangeOffset);
            ...
            try {
                ...
                for (int i = 0; i < cpuQuarks.size(); i++) {
                    ...
                    for (TmfTimeRange queryingInterval : queryingIntervals) {
                        ...
                        newContainer.addCPUContextSwitches(i, queryingInterval, threadChangesInTimeRange.size() - 1L);
                    }
                }
