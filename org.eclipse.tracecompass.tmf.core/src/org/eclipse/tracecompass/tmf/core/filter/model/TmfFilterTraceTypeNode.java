...
public class TmfFilterTraceTypeNode extends TmfFilterTreeNode {
     ...
     */
    public void setTraceClass(Class<?> traceClass) {
    ...
    public boolean matches(ITmfEvent event) {
        boolean match = false;
        if (event.getTrace().getClass().equals(fTraceClass)) {
            if (fTraceClass == CustomTxtTrace.class || fTraceClass == CustomXmlTrace.class) {
                // special case for custom parsers
                if (fTraceTypeId.endsWith(event.getType().getName())) {
                    match = true;
                }
            } else {
                match = true;
