    ...
    public boolean matchEvents() {
        ...
        Job job = new Job(Messages.TmfEventMatching_MatchingEvents) {
            ...
            protected IStatus run(final IProgressMonitor monitor) {
                 ...
                 * </pre>
                 */
                SubMonitor subMonitor = SubMonitor.convert(monitor, fTraces.size());
                for (ITmfTrace trace : fTraces) {
                    subMonitor.beginTask(NLS.bind(Messages.TmfEventMatching_LookingEventsFrom, trace.getName()), IProgressMonitor.UNKNOWN);
                    SubMonitor childMonitor = subMonitor.newChild(1);

                    /* Send the request to the trace */
                    EventMatchingBuildRequest request = new EventMatchingBuildRequest(TmfEventMatching.this, trace, childMonitor);
