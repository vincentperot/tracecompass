...
public class TmfEventFieldAspect implements ITmfEventAspect {
    ...
    public @Nullable String resolve(ITmfEvent event) {
        ITmfEventField root = getRootField(event);
        if (root == null) {
            return null;
        }
        ITmfEventField field;
        if (fFieldArray == null) {
            field = root.getField(fFieldName);
        } else {
    ...
    /**
     * Returns the root event field for this aspect. The default implementation
     * returns the event content. Subclasses may override to provide another
     * event member but should not assume the event is of any specific type.
     *
     * @param event
     *            The event to process
     * @return the root event field
     */
    protected @Nullable ITmfEventField getRootField(ITmfEvent event) {
