...
public class TmfContentFieldAspect implements ITmfEventAspect {
     ...
     * @param fieldPath
     *            The field name or field path array to look for in the event
     *            content. Should *not* be localized!
     */
    public TmfContentFieldAspect(String aspectName, String... fieldPath) {
        fAspectName = aspectName;
        fFieldArray = checkNotNull(Arrays.copyOf(fieldPath, fieldPath.length));
    }
    ...
    public @Nullable Object resolve(ITmfEvent event) {
        ITmfEventField field = event.getContent().getField(fFieldArray);
        if (field == null) {
            return null;
        }
