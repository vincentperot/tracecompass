...
public final class CTFStreamInputPacketReader implements IPacketReader {
    ...
    @NonNull
    private final ICTFPacketInformation fCurrentPacket;
     ...
     * @since 1.0
     */
    public CTFStreamInputPacketReader(@NonNull ICTFPacketInformation currentPacket, CTFStream stream, CTFStreamInput streamInput, CTFStreamInputReader streamInputReader, @NonNull ByteBuffer bb) throws CTFException {
        fStream = stream;
        ...
        if (fCurrentStreamPacketContextDef != null) {
            ...
            if (fLostEventsInThisPacket != 0) {
                ...
                int index = streamInput.getIndex().indexOf(currentPacket);
                if (index == 0) {
                    lostEventsStartTime = currentPacket.getTimestampBegin() + 1;
                } else {
                    ICTFPacketInformation prevPacket = streamInput.getIndex().getElement(index - 1);
     ...
     */
    @Override
    public final ICTFPacketInformation getPacketInformation() {
