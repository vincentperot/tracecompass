...
public class CTFStreamInput implements IDefinitionScope {
    ...
    /**
     * Gets the first read timestamp of a stream. (this is usually the fist time
     * in the stream.)
     *
     * @return the first timestamp
     */
    public long getTimestampStart() {
        return getIndex().getEntries().get(0).getTimestampBegin();
    }
    ...
    private void parseTracePacketHeader(StructDeclaration tracePacketHeaderDecl,
        ...
        ICompositeDefinition tracePacketHeaderDef = tracePacketHeaderDecl.createDefinition(fStream.getTrace(), LexicalScope.TRACE_PACKET_HEADER, bitBuffer);
    ...
    private void parsePacketContext(long fileSizeBytes,
        ...
        ICompositeDefinition streamPacketContextDef = streamPacketContextDecl.createDefinition(this, LexicalScope.STREAM_PACKET_CONTEXT, bitBuffer);
    ...
    /**
     * Gets the first timestamp of the stream, may be before an event
     *
     * @return gets the first timestamp of the stream.
     */
    public long getTimetstampBegin() {
        return getIndex().getEntries().get(0).getTimestampBegin();
    }
