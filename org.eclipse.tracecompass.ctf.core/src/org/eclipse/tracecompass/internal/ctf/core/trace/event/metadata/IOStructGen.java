...
package org.eclipse.tracecompass.internal.ctf.core.trace.event.metadata;
...
import org.eclipse.tracecompass.internal.ctf.core.types.IEventHeaderDeclaration;
...
import org.eclipse.tracecompass.internal.ctf.core.types.StructDeclarationFlattener;
...
public class IOStructGen {
    ...
    private static final Set<ICompositeDeclaration> ACCELERATORS = new HashSet<>();

    /**
     * Add an accelerator for composite declarations
     *
     * @param declaration
     *            the declaration to add
     */
    public static synchronized void addAccelerator(ICompositeDeclaration declaration) {
        ACCELERATORS.add(declaration);
    }
    ...
    public IOStructGen(CommonTree tree, CTFTrace trace) {
        fTrace = trace;
        fTree = tree;
        IOStructGen.addAccelerator(EventHeaderCompactDeclaration.getEventHeader(ByteOrder.BIG_ENDIAN));
        IOStructGen.addAccelerator(EventHeaderCompactDeclaration.getEventHeader(ByteOrder.LITTLE_ENDIAN));
        IOStructGen.addAccelerator(EventHeaderLargeDeclaration.getEventHeader(ByteOrder.BIG_ENDIAN));
        IOStructGen.addAccelerator(EventHeaderLargeDeclaration.getEventHeader(ByteOrder.LITTLE_ENDIAN));
    }
    ...
    private void parseEventDeclaration(CommonTree eventDecl,
        ...
        } else if (left.equals(MetadataStrings.LOGLEVEL)) {
    ...
    private IDeclaration parseTypeSpecifierList(CommonTree typeSpecifierList,
        ...
        case CTFParser.STRUCT:
            ...
            /* replace with accelerator if possible */
            for (ICompositeDeclaration accelerator : ACCELERATORS) {
                if (accelerator.isBinaryEquivalent(declaration)) {
    ...
    private IntegerDeclaration parseInteger(CommonTree integer)
        ...
        @NonNull
        String clock = NonNullUtils.nullToEmptyString(null);
        ...
        for (CommonTree child : children) {
            ...
            case CTFParser.CTF_EXPRESSION_VAL:
                ...
                if (left.equals(MetadataStrings.SIGNED)) {
                    signed = getSigned(rightNode);
                } else if (left.equals(MetadataStrings.BYTE_ORDER)) {
                    byteOrder = getByteOrder(rightNode);
                } else if (left.equals(MetadataStrings.SIZE)) {
                    size = getSize(rightNode);
                } else if (left.equals(MetadataStrings.ALIGN)) {
                    alignment = getAlignment(rightNode);
                } else if (left.equals(MetadataStrings.BASE)) {
                    base = getBase(rightNode);
                } else if (left.equals(MetadataStrings.ENCODING)) {
                    encoding = getEncoding(rightNode);
                } else if (left.equals(MetadataStrings.MAP)) {
    ...
    private static String getClock(CommonTree rightNode) {
        ...
        return clock == null ? NonNullUtils.nullToEmptyString(null) : clock;
    }
