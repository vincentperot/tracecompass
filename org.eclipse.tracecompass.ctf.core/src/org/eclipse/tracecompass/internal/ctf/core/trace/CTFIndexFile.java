...
public class CTFIndexFile {
    ...
    private static final int PACKET_SIZE = 7 * Long.SIZE / 8;
    ...
    public CTFIndexFile(File indexFile) throws CTFReaderException {
        try (DataInputStream dataIn = new DataInputStream(new FileInputStream(indexFile))) {
            ...
            while (dataIn.available() >= PACKET_SIZE) {
                ...
                if (header.oldMagic) {
                    element = new StreamInputPacketIndexEntry(dataIn, indexFile.getName().replaceAll("\\.idx", "")); //$NON-NLS-1$ //$NON-NLS-2$
                } else {
                    element = new StreamInputPacketIndexEntry(dataIn);
                }
                pies.add(element);
            }
