...
public class LttngKernelStateProvider extends AbstractTmfStateProvider {
    ...
    private static final int SYSCALL_EXIT_IDX = 0;
    private static final int IRQ_HANDLER_ENTRY_IDX = 1;
    private static final int IRQ_HANDLER_EXIT_IDX = 2;
    private static final int SOFT_IRQ_ENTRY_IDX = 3;
    private static final int SOFT_IRQ_EXIT_IDX = 4;
    private static final int SOFT_IRQ_RAISE_IDX = 5;
    private static final int SCHED_SWITCH_IDX = 6;
    private static final int SCHED_PROCESS_FORK_IDX = 7;
    private static final int SCHED_PROCESS_EXIT_IDX = 8;
    private static final int SCHED_PROCESS_FREE_IDX = 9;
    private static final int STATEDUMP_PROCESS_STATE_IDX = 10;
    private static final int SCHED_WAKEUP_IDX = 11;
    ...
    private static Map<String, Integer> buildEventNames(IKernelAnalysisEventLayout layout) {
        ...
        builder.put(layout.eventSyscallExit(), SYSCALL_EXIT_IDX);
        builder.put(layout.eventIrqHandlerEntry(), IRQ_HANDLER_ENTRY_IDX);
        builder.put(layout.eventIrqHandlerExit(), IRQ_HANDLER_EXIT_IDX);
        builder.put(layout.eventSoftIrqEntry(), SOFT_IRQ_ENTRY_IDX);
        builder.put(layout.eventSoftIrqExit(), SOFT_IRQ_EXIT_IDX);
        builder.put(layout.eventSoftIrqRaise(), SOFT_IRQ_RAISE_IDX);
        builder.put(layout.eventSchedSwitch(), SCHED_SWITCH_IDX);
        builder.put(layout.eventSchedProcessFork(), SCHED_PROCESS_FORK_IDX);
        builder.put(layout.eventSchedProcessExit(), SCHED_PROCESS_EXIT_IDX);
        builder.put(layout.eventSchedProcessFree(), SCHED_PROCESS_FREE_IDX);
        builder.put(layout.eventStatedumpProcessState(), STATEDUMP_PROCESS_STATE_IDX);

        for (String eventSchedWakeup : layout.eventsSchedWakeup()) {
            builder.put(eventSchedWakeup, SCHED_WAKEUP_IDX);
        }
    ...
    protected void eventHandle(ITmfEvent event) {
        ...
        try {
            ...
            case SCHED_PROCESS_EXIT_IDX:
                break;

            case SCHED_PROCESS_FREE_IDX:
            {
            ...
            case STATEDUMP_PROCESS_STATE_IDX:
                /* LTTng-specific */
            {
                ITmfEventField content = event.getContent();
