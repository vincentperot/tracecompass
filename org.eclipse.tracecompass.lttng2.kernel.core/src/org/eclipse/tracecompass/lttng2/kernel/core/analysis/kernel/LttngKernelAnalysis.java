...
//            LttngStrings.SYSCALL_PREFIX
     ...
     *         <code>-1</code> if either no thread is running or we do not know.
     */
    public int getThreadOnCpu(long cpuId, long ts) {
        ...
        try {
            int cpuQuark = stateSystem.getQuarkAbsolute(Attributes.CPUS, Long.toString(cpuId), Attributes.CURRENT_THREAD);
            ITmfStateInterval interval = stateSystem.querySingleState(ts, cpuQuark);
    ...
    public @NonNull Collection<Integer> getThreadQuarks() {
        ...
        try {
            ...
            Activator.getDefault().logError("LTTng Kernel analysis: Attribute 'Threads' not found"); //$NON-NLS-1$
        }
        @SuppressWarnings("null")
        @NonNull
        Collection<Integer> empty = Collections.EMPTY_SET;
     ...
     * @return The parent PID or {@code -1} if the PPID is not found.
     */
    public @NonNull Integer getPpid(@NonNull Integer proc, long ts) {
        Integer ppid = -1;
        ITmfStateSystem ss = getStateSystem();
        if (ss == null) {
            return ppid;
        ...
        Integer ppidNode;
        try {
            ppidNode = ss.getQuarkAbsolute(Attributes.THREADS, proc.toString(), Attributes.PPID);
            ...
            case INTEGER:
                ppid = ppidValue.unboxInt();
                /*
                 * FIXME: I need this because otherwise there is a null safety
                 * warning on ppid... why?
                 */
                if (ppid == null) {
    ...
    public @NonNull List<ITmfStateInterval> getStatusIntervalsForThread(int threadQuark, long start, long end, long resolution, IProgressMonitor monitor) {
        ...
        try {
            int statusQuark = ssq.getQuarkRelative(threadQuark, Attributes.STATUS);
            List<ITmfStateInterval> statusIntervals = StateSystemUtils.queryHistoryRange(ssq, statusQuark, Math.max(start, ssq.getStartTime()), Math.min(end - 1, ssq.getCurrentEndTime()), resolution, monitor);
