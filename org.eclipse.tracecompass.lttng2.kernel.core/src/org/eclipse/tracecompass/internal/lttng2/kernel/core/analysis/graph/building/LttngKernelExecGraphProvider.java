...
public class LttngKernelExecGraphProvider extends AbstractTmfGraphProvider {
    ...
    public void done() {
        ...
        Set<Object> keys = graph.getObjects();
            ...
            for(TmfVertex node: nodes) {
                ...
                if (node.hasNeighbor(TmfVertex.INV) &&
                    ...
                    if (!next.hasNeighbor(TmfVertex.OUTV) && node.hasNeighbor(TmfVertex.OUTH)) {
                        next = NonNullUtils.checkNotNull(node.neighbor(TmfVertex.OUTH));
                    }
                    TmfVertex dst = next.outv();
                    if (dst != null) {
                        TmfVertex src = NonNullUtils.checkNotNull(node.neighbor(TmfVertex.INV));
                        /* unlink */
