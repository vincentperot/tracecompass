...
public class TraceEventHandlerSched extends AbstractTraceEventHandler {
    ...
    public void handleEvent(ITmfEvent ev) {
        ...
        } else if (LttngStrings.SCHED_WAKEUP.equals(eventName) && !fHasEventSchedTTWU.get(ev.getTrace())) {
            handleSchedWakeup(ev);
        } else if (LttngStrings.SCHED_WAKEUP_NEW.equals(eventName) && !fHasEventSchedTTWU.get(ev.getTrace())) {
            handleSchedWakeup(ev);
        } else if (LttngStrings.SCHED_TTWU.equals(eventName) && fHasEventSchedTTWU.get(ev.getTrace())) {
            handleSchedWakeup(ev);
    ...
    private void handleSchedWakeup(ITmfEvent event) {
        ...
        System.err.println("sched_wakeup target " + target + " in invalid state: " + target.getStatus() + " at time " + event.getTimestamp()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
