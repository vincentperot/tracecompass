...
public class TraceEventHandlerExecutionGraph extends AbstractTraceEventHandler {
    ...
    private TmfNetworkEventMatching fTcpMatching;
     ...
     * Constructor
     *
     * @param provider The graph provider
     * @param system The system model associated with the system under investigation
     */
    public TraceEventHandlerExecutionGraph(AbstractTmfGraphProvider provider, LttngSystemModel system) {
        ...
        Collection<ITmfTrace> traces = TmfTraceManager.getTraceSet(provider.getTrace());
        fTcpMatching = new TmfNetworkEventMatching(traces, fMatchProcessing);
    ...
    private void handleSchedWakeup(ITmfEvent event) {
        ...
        case WAIT_BLOCKED:
            ...
            if (context == null) {
                context = LttngInterruptContext.DEFAULT_CONTEXT;
            }
    ...
    private void handleInetSockLocalIn(ITmfEvent event) {
        ...
        if (context == Context.SOFTIRQ) {
            ...
            fTcpMatching.matchEvent(event, event.getTrace());
        }
    }
