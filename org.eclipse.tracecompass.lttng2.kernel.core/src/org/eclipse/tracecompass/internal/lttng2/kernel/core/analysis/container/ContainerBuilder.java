...
public class ContainerBuilder {
    ...
    private boolean isParentContainerAdded(ITmfStateSystemBuilder ssb, TaskContainerEvent task) {
        ...
        if (task.getNsInode() != rootContainerInfo.getContainerInode()) {
            TaskContainerEvent parent = fTasks.get(task.getPpid());
            int parentContainerQuark = ContainerHelper.findContainerQuarkWithInode(ssb, parent.getNsInode());
            if (parentContainerQuark == ContainerHelper.fCONTAINER_QUARK_NOT_FOUND) {
                /*
                 * We cannot find the container. This can happen, and means that
                 * the parent container was not added yet.
                 */
                return false;
            }
        }
    ...
    private void processTaskEvent(ITmfStateSystemBuilder ssb, TaskContainerEvent t) {
        ...
        if (parent != null && t.getNsInode() != rootContainerInfo.getContainerInode()) {
            ...
            if (taskContainerQuark != ContainerHelper.fCONTAINER_QUARK_NOT_FOUND) {
                ContainerHelper.addTaskToContainer(ssb, taskContainerQuark, t.getTs(), t.getVtid(), t.getTid());
            } else {
