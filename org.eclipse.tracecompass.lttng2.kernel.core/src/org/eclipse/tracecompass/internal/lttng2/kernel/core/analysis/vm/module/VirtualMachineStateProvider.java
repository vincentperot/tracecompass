...
public class VirtualMachineStateProvider extends AbstractTmfStateProvider {
    ...
    public VirtualMachineStateProvider(TmfExperiment experiment) {
        ...
        @NonNull
        Table<ITmfTrace, String, Integer> table = HashBasedTable.create();
        fEventNames = table;
    ...
    protected void eventHandle(@Nullable ITmfEvent event) {
        ...
        // if (!VirtualMachineCpuAnalysis.REQUIRED_EVENTS.contains(eventName) &&
        ...
        try {
             ...
             */
            {
                ...
                if (vcpu != null) {
                    ...
                    int curStatusQuark = ss.getQuarkRelativeAndAdd(getNodeVirtualMachines(), vm.getHostId(),
                            vcpu.getCpu().toString(), VmAttributes.STATUS);
            ...
            /* Other event types not covered by the main switch */
            {
                ...
                if (virtualCpu != null) {
                    ...
                    int curStatusQuark = ss.getQuarkRelativeAndAdd(getNodeVirtualMachines(), vm.getHostId(),
                            Long.toString(virtualCpu.getCpu()), VmAttributes.STATUS);
                    value = ss.queryOngoingState(curStatusQuark);
                    int newVal = Math.max(VcpuStateValues.VCPU_UNKNOWN, value.unboxInt());
    ...
    private @Nullable HostThread getCurrentHostThread(ITmfEvent event, long ts) {
        ...
        int currentTid = module.getThreadOnCpu(cpu, ts);
        if (currentTid == LttngKernelAnalysis.NO_TID) {
            return null;
        }
