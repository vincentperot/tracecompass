    ...
    public Multimap<String, ITmfStateInterval> getUpdatedThreadIntervals(int vmQuark, long start, long end, long resolution, IProgressMonitor monitor) {
        ...
        final Multimap<String, ITmfStateInterval> map = TreeMultimap.create(new Comparator<String>() {
            ...
            public int compare(@Nullable String arg0, @Nullable String arg1) {
                /* Keys do not have to be sorted, just use natural sorting */
            ...
            public int compare(@Nullable ITmfStateInterval arg0, @Nullable ITmfStateInterval arg1) {
                ...
                if (arg1.getStateValue() == VCPU_PREEMPT_VALUE && arg0.getStateValue() != VCPU_PREEMPT_VALUE) {
                    /*
                     * For VCPU_PREEMPT state values, the state has to be after
                     * any other state that overlaps it
                     */
            ...
            for (Integer vcpuQuark : ss.getSubAttributes(vmQuark, false)) {
                ...
                for (ITmfStateInterval cpuInterval : StateSystemUtils.queryHistoryRange(ss, statusQuark, start, end - 1, resolution, monitor)) {
                    ...
                    case INTEGER:
                        ...
                        int threadOnCpu = kernelModule.getThreadOnCpu(virtualCPU, cpuInterval.getStartTime());
