...
public class SyslogTrace extends TextTrace<SyslogEvent> {
    ...
    public interface Field {
        @NonNull String TIMESTAMP = "Timestamp";
    ...
    public static final @NonNull Collection<ITmfEventAspect> ASPECTS =
            checkNotNull(ImmutableList.of((ITmfEventAspect)
                    new TmfContentFieldAspect(Field.TIMESTAMP, Field.TIMESTAMP),
    ...
    protected SyslogEvent parseFirstLine(Matcher matcher, String line) {
        ...
        TextTraceEventContent content = new TextTraceEventContent(ASPECTS.size());
        content.setValue(new StringBuffer(line));
        content.addField(Field.TIMESTAMP, matcher.group(1));
