...
public class CriticalPathAlgorithmBounded extends AbstractCriticalPathAlgorithm {
    ...
    public TmfGraph compute(TmfVertex start, @Nullable TmfVertex end) {
        ...
        while (nextVertex != null && (nextVertex.getTs() < endTime)) {
            ...
            nextVertex = currentVertex.outgoingHorizontalEdge();
        }
        return criticalPath;
    }
    ...
    private void glue(TmfGraph criticalPath, TmfGraph graph, TmfVertex currentVertex, List<TmfEdge> links) {
        ...
        if (links.isEmpty()) {
             ...
             */
            TmfVertex next = currentVertex.neighbor(TmfVertex.OUTGOING_HORIZONTAL_EDGE);
            if (next == null) {
                return;
            }
            criticalPath.append(currentActor, new TmfVertex(next), currentVertex.getEdges().get(TmfVertex.OUTGOING_HORIZONTAL_EDGE).getType());
            return;
        }
    ...
    private List<TmfEdge> resolveBlockingBounded(TmfEdge blocking, TmfVertex bound) {
        ...
        while (vertexFrom != null && vertexFrom.compareTo(currentBound) > 0) {
            ...
            TmfVertex inVertical = vertexFrom.incomingVerticalEdge();
            if (inVertical != null && inVertical.compareTo(currentBound) <= 0) {
                subPath.add(vertexFrom.getEdges().get(TmfVertex.INCOMING_VERTICAL_EDGE));
                break;
            }
             ...
             */
            TmfVertex inHorizontal = vertexFrom.incomingHorizontalEdge();
            if (inVertical != null &&
                    (inHorizontal == null ||
                            vertexFrom.getEdges().get(TmfVertex.INCOMING_HORIZONTAL_EDGE).getType() != TmfEdge.EdgeType.BLOCKED ||
                    vertexFrom.getEdges().get(TmfVertex.INCOMING_HORIZONTAL_EDGE).getType() != TmfEdge.EdgeType.NETWORK)) {
                stack.push(vertexFrom);
            }
            if (inHorizontal != null) {
                TmfEdge link = vertexFrom.getEdges().get(TmfVertex.INCOMING_HORIZONTAL_EDGE);
                ...
                if (!stack.isEmpty()) {
                    ...
                    subPath.add(v.getEdges().get(TmfVertex.INCOMING_VERTICAL_EDGE));
                    vertexFrom = v.neighbor(TmfVertex.INCOMING_VERTICAL_EDGE);
                    continue;
                }
            }
